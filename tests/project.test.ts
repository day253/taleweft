...
test("anchored sound follows its text when paragraphs are reordered", () => {
  ...
  assert.equal(
    cueStart(c, bell),
    segmentTimings(c).find((s) => s.id === "s3")!.start,
  );
  const target = c.segments.splice(2, 1)[0];
  c.segments.unshift(target);
...
test("plain text import preserves sentences and extracts explicit speaker labels", () => {
  ...
  assert.deepEqual(
    p.chapters.flatMap((c) => c.segments.map((s) => s.text)),
    ["你听见了吗？", "雨一直在下。", "天亮了。"],
  );
  assert.deepEqual(
...
test("music range remains bound to the same text when a segment length changes", () => {
  const chapter = structuredClone(seedProject.chapters[0]);
  const music = chapter.cues.find((c) => c.id === "piano")!;
  const before = cueDuration(chapter, music);
  chapter.segments.find((s) => s.id === "s5")!.duration += 5;
  assert.equal(cueDuration(chapter, music), before + 5);
});
test("local proposal changes only its selected segment and leaves original project immutable", () => {
  ...
  assert.throws(
    () =>
      applyPatches(seedProject, [
        { segmentId: "s2", changes: { characterId: "missing" } },
      ]),
    /角色不存在/,
  );
});
